...
class LoRA(nn.Module):
    def __init__(self, original_layer, r=4, alpha=32):
        ...
        self.A = nn.Parameter(torch.zeros(original_layer.in_features, r))
        self.B = nn.Parameter(torch.zeros(r, original_layer.out_features))
    ...
    def forward(self, x):
        ...
        h += x @ (self.A @ self.B) * self.scaling
        return h
...
def inject_lora_into_model(model, r=4, alpha=32, device='cpu'):
    ...
    for child_name, child_module in model.named_children():
        ...
        if child_name in ['q', 'k', 'v', 'o'] and isinstance(child_module, nn.Linear):
...
class SoftPromptEmbedding(nn.Module):
    ...
    def forward(self, input_embeddings):
        ...
        soft_prompt_expanded = self.soft_prompt.unsqueeze(0).expand(batch_size, -1, -1).to(input_embeddings.device)
